...
class SmbCopierBase(Processor):
    """Base class with shared functionality for all processors."""
    
    # Global version
    __version__ = "2026.07.21.0"
    ...
    def try_smbcopy(
        ...
        try:
            ...
            if not self.filesystem[src_loc]['exists'](source_path):
                self.output(f"Source does not exist: {source_path}")
                raise ProcessorError('Source does not exist.')
            
            src_type = 'file' if self.filesystem[src_loc]['isfile'](source_path) else \
                'directory' if self.filesystem[src_loc]['isdir'](source_path) else \
                    None
            ...
            if self.filesystem[dst_loc]['should_nest'](destination_path):
                self.output(f"Source {src_type} will be nested into destination path.", 3)
                _destination_path = self.filesystem[dst_loc]['join'](
                    destination_path,
                    self.convert_path(
                        path=self.filesystem[src_loc]['split'](source_path)[-1],
                        in_type=src_loc,
                        out_type=dst_loc
                        )
                    )
            ...
            if self.filesystem[dst_loc]['exists'](_destination_path):
                ...
                if not overwrite:
                    ...
                    if src_type == 'file':
                        self.filesystem[dst_loc]['remove'](_destination_path)
                    else:
                        for root,dirs,files in self.filesystem[dst_loc]['walk'](_destination_path,topdown=False):
                            for file in files:
                                self.output(f"Deleting File: {self.filesystem[dst_loc]['join'](root,file)}", 3)
                                self.filesystem[dst_loc]['remove'](self.filesystem[dst_loc]['join'](root,file))
                            for dir in dirs:
                                self.output(f"Deleting Directory: {self.filesystem[dst_loc]['join'](root,dir)}", 3)
                                self.filesystem[dst_loc]['rmdir'](self.filesystem[dst_loc]['join'](root,dir))
                        self.output(f"Deleting Directory: {root}", 3)
                        self.filesystem[dst_loc]['rmdir'](root)
                        self.output("Finished deleting current occupants.", 3)

            dirname = self.filesystem[dst_loc]['parent'](_destination_path)
            if not self.filesystem[dst_loc]['exists'](dirname):
                if create_path:
                    self.output(f"Creating parent directory, if it does not exist: {dirname}", 3)
                    _ = self.filesystem[dst_loc]['makedirs'](dirname, exist_ok=True)
                else:
            ...
            if (src_type == 'directory'):
                ...
                for root, dirs, files in self.filesystem[src_loc]['walk'](source_path):
                    if src_root is None:
                        src_root = root
                    for dir in dirs:
                        src_dir_relpath = self.filesystem[src_loc]['relpath'](self.filesystem[src_loc]['join'](root,dir),src_root)
                        src_dir_abspath = self.filesystem[src_loc]['join'](root,dir)
                        src_dir_stats = self.filesystem[src_loc]['stat'](src_dir_abspath)
                        src_dir_atime = src_dir_stats.st_atime
                        src_dir_mtime = src_dir_stats.st_mtime
                        
                        dst_dir_abspath = self.filesystem[dst_loc]['join'](dst_root,src_dir_relpath)
                        self.output(f"Creating directory: {src_dir_relpath}", 3)
                        self.filesystem[dst_loc]['makedirs'](dst_dir_abspath,exist_ok=True)
                        self.output("Setting modification/access time attributes on directory", 3)
                        self.filesystem[dst_loc]['utime'](dst_dir_abspath,times=(src_dir_atime,src_dir_mtime))

                    for file in files:
                        src_file_relpath = self.filesystem[src_loc]['relpath'](self.filesystem[src_loc]['join'](root,file),src_root)
                        dst_file_relpath = self.convert_path(src_file_relpath,in_type=src_loc,out_type=dst_loc)
                        src_file_abspath = self.filesystem[src_loc]['join'](source_path,src_file_relpath)
                        dst_file_abspath = self.filesystem[dst_loc]['join'](dst_root,dst_file_relpath)

                        src_file_stats = self.filesystem[src_loc]['stat'](src_file_abspath)
                        ...
                        with self.filesystem[src_loc]['opener'](src_file_abspath,mode='rb') as src, \
                            self.filesystem[dst_loc]['opener'](dst_file_abspath,mode='wb') as dst:
                            shutil.copyfileobj(src,dst)
                        
                        self.output("Setting modification/access time attributes on file", 3)
                        self.filesystem[dst_loc]['utime'](dst_file_abspath,times=(src_file_atime,src_file_mtime))
                    self.output("Finished copying items")
            else:
                self.output(f"Copying file {source_path} to {_destination_path}")
                with self.filesystem[src_loc]['opener'](source_path,mode='rb') as src, \
                    self.filesystem[dst_loc]['opener'](_destination_path,mode='wb') as dst:
                    shutil.copyfileobj(src,dst)
                src_file_stats = self.filesystem[src_loc]['stat'](source_path)
                src_file_atime = src_file_stats.st_atime
                src_file_mtime = src_file_stats.st_mtime
                self.output("Setting modification/access time attributes on file", 3)
                self.filesystem[dst_loc]['utime'](_destination_path,times=(src_file_atime,src_file_mtime))
    ...
    def check_smb_path(self,smb_path):
        try:
            self.smb_path_exists = None
